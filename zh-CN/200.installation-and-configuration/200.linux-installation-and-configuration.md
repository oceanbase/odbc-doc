...
# rpm -qa | grep obclient
obclient-2.2.4-20231027102957.el7.x86_64
...
```
# rpm -ev ob-unixodbc-<version_no>.rpm
# rpm -ev ob-connector-odbc-<version_no>.rpm   
# rpm -ev libobclient-<version_no>.rpm   
```

如未安装，请参见下面章节，首先安装软件依赖包，再安装 OB ODBC 驱动。
...
完成安装软件依赖包 Libtool 后，你可以依次执行下面的命令安装 OB ODBC 驱动。
...
1. 配置 `odbc.ini` 到 `/etc/odbc.ini`（也可以自定义目录设置环境变量到相应的目录即可），执行下面的命令，编辑 odbc.ini 配置文件。
...
2. 点击键盘上的 **i** 进入编辑状态，将下面的内容添加到文件内。需要注意的是，`[Oceanbase]`前面不能有空格。
...
3. 点击键盘上的 **esc** 保存，输入 **:wq** 退出文件。

### 配置 odbcinst.ini

1. 配置 `odbcinst.ini` 到 `/etc/odbcinst.ini`（也可以自定义目录设置环境变量到相应的目录即可）。执行下面的命令，编辑 odbc.ini 配置文件。
