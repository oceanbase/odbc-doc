...
如未安装，请参见下面章节，首先安装软件依赖包，再安装 OceanBase ODBC 驱动。
...
完成安装软件依赖包 Libtool 后，你可以依次执行下面的命令安装 OceanBase ODBC 驱动。
...
配置 `odbcinst.ini` 到 `/etc/odbcinst.ini`（也可以自定义目录设置环境变量到相应的目录即可）。执行下面的命令，编辑 odbcinst.in 配置文件。
