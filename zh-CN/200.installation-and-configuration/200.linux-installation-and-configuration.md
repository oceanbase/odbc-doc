...
最后，点击键盘上的 **esc** 保存，输入 **:wq** 退出文件。

### 配置 odbcinst.ini

配置 `odbcinst.ini` 到 `/etc/odbcinst.ini`（也可以自定义目录设置环境变量到相应的目录即可）。执行下面的命令，编辑 odbc.ini 配置文件。
...
文件内容如下所示，您只需将示例中的 `DSN=oboracle` 改为实际的 DSN 即可，例如，您可以改为本文 `/etc/odbc.ini` 中的 `OBDSN`：：
