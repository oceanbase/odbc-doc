...
1. 检查是否已安装 OceanBase Connector/ODBC，并查看相应版本信息。
   ...
   ```
   # rpm -qa | grep odbc
   ob-connector-odbc-2.0.8.2-20230719101407.el7.alios7.x86_64
   ob-unixodbc-2.0.8.2-20230719100704.el7.alios7.x86_64
   ...
   # rpm -ivh ob-connector-odbc-2.0.8.2-20230719101407.el7.alios7.x86_64.rpm
   # rpm -ivh ob-unixodbc-2.0.8.2-20230719100704.el7.alios7.x86_64.rpm
   ```
...
此外，根据实际环境还需要安装 Libtool 相关工具：libtool-ltdl 和 libreadline。

### 安装 libtool-ltdl

1. 检查是否已安装 libtool-ltdl。
...
2. 如未安装，请执行下面的命令安装。
   ...
   ```
   yum search libtool-ltdl
   yum install libtool-ltdl
   ```

### 安装 libreadline

1. 检查是否已安装 libreadline。
...
1. 执行下面的命令检查 libobodbc 依赖。
   ...
   ```
   #ldd /u01/ob-connector-odbc/lib64/mariadb/libobodbc.so
	   linux-vdso.so.1 (0x00007fffc0bda000)
	   libobclnt.so.20 => /u01/obclient/lib/libobclnt.so.20 (0x00007f71c26af000)
	   libodbcinst.so.2 => /u01/unix-odbc/lib/libodbcinst.so.2 (0x00007f71c2497000)
	   libm.so.6 => /usr/lib64/libm.so.6 (0x00007f71c22f8000)
	   libc.so.6 => /usr/lib64/libc.so.6 (0x00007f71c2140000)
	   libdl.so.2 => /usr/lib64/libdl.so.2 (0x00007f71c213b000)
	   libpthread.so.0 => /usr/lib64/libpthread.so.0 (0x00007f71c2118000)
	   libltdl.so.7 => /usr/lib64/libltdl.so.7 (0x00007f71c210d000)
	   /lib64/ld-linux-x86-64.so.2 (0x00007f71c2baa000)
   ```
